...
from rnd_model import RndRewardModel
...
class MaskablePPO(OnPolicyAlgorithm):
    ...
    :param: rnd_config: config for building a rnd reward model
    """
    ...
    def __init__(
        ...
        if self.rnd_config is not None:
            self.rnd_config["observation_shape"] = self.env.observation_space.n
            try:
                self.rnd_model = RndRewardModel(self.rnd_config)
            except:
                raise ValueError("Incorrect rnd config.")
        else:
            self.rnd_model = None
    ...
    def update_rnd_model(self) -> None:
        """
        Update RND model.
        """
        batch_num, env_num, obs_dim = self.rollout_buffer.observations.shape
        obs = self.rollout_buffer.observations.reshape(-1, obs_dim)
        self.rnd_model.train(obs)
        
    def add_rnd_reward(self) -> None:
        """
        Add rnd reward to 
        """
        batch_num, env_num, obs_dim = self.rollout_buffer.observations.shape
        obs = self.rollout_buffer.observations.reshape(-1, obs_dim)
        rnd_reward = self.rnd_model.estimate(obs).reshape(batch_num, env_num)
        self.rollout_buffer.rewards += rnd_reward

    def train(self) -> None:
        """
        Update RND model and policy using the currently gathered rollout buffer.
        """
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size): # rollout_data has already sent to device
